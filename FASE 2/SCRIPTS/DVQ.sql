lotes y ensayos
--AGREGO LAS 2 COLUMNAS PARA GUARDAR LOS IDS MAXIMOS POR AÑO TANTO DE LOTE COMO DE 
...
INSERT INTO "CPSAA"."GESAC_DET_PERM_ROL"(
    ...
    VALUES ((SELECT coalesce(max(PER."SCDPR_IDE_PERM_ROL_K"),0) + 1 FROM "CPSAA"."GESAC_DET_PERM_ROL" PER), (SELECT MAX("SCMAP_IDE_PERM_K") FROM "CPSAA"."GESAC_MAE_PERM"), 11,'N', 'ACT');
...
LAS PALABRAS: RESISTENCIA, COMPRESIÓN o COMPRESION Y 28
...
DECLARE
	...
	IF(posicion > 0) THEN select position('COMPRESIÓN' in upper(nombreTipoEnsayo)) into posicion;
		IF(posicion > 0) THEN 
...
-- Function: verificarlotepnc()-- DROP FUNCTION verificarlotepnc();CREATE OR REPLACE FUNCTION verificarlotepnc()  RETURNS trigger AS$BODY$/**********************************************FECHA		: 21-03-2012DESCRIPCION	: CAMBIA DE ESTADO EL LOTE A NO CONFORME E INSERTA EN LA TABLA LOTE_PNCAUTOR		: DVQ**********************************************/    DECLARE		idLote numeric(10,0); --GUARDA EL ID DEL LOTE PARA REALIZAR EL UPDATE Y EL INSERT	nombreTipoEnsayo character varying(100);	insertoLotePNC integer := 0;	contadorEstadoNOC integer := 0;	idPlanta numeric(10,0); --GUARDA EL ID DE LA PLANTA DEL LOTE	--DVQ 22-03-2012	--existeResistenciaCompresion integer := 0; --> 0: SI NO EXISTE RESISTENCIA COMPRESION 28 DIAS, 1: SI EXISTE Y SU ESTADO ES 'NOC', 2: SI EXISTE Y SU ESTADO ES DIFERENTE A 'NOC'	contadorLoteEnLotePNC integer := 0; --> SIRVE PARA SABER SI EN LA TABLA DE LOTE_PNC YA HAY UN REGISTRO CON EL IDLOTE (PARA NO REGISTRAR 2 VECES CON EL MISMO ID LOTE)	esResistencia28Dias integer := 0; --> GUARDA EL FLAG PARA SABER SI EL NOMBRE DEL TIPO DE ENSAYO ES DE RESISTENCIA COMPRESION 28 DIAS, 1 = ES DE RESISTENCIA COMPRESION, 0 = NO ES    BEGIN		-- OBTENGO EL ID DEL LOTE AL QUE ESTA ASOCIADO ESE ENSAYO	  SELECT 		LOTE."SCMOL_IDE_LOTE_K" INTO idLote	  FROM		"CPSAA"."GESAC_MOV_LOTE" LOTE INNER JOIN		"CPSAA"."GESAC_MOV_MUES" MUESTRA ON		LOTE."SCMOL_IDE_LOTE_K" = MUESTRA."SCMOL_IDE_LOTE_K" INNER JOIN		"CPSAA"."GESAC_MOV_ENSA" ENSAYO ON		ENSAYO."SCMOM_IDE_MUES_K" = MUESTRA."SCMOM_IDE_MUES_K"	  WHERE		ENSAYO."SCMOE_IDE_ENSA_K" = NEW."SCMOE_IDE_ENSA_K";				--VERIFICO QUE idLote NO SEA NULO	IF idLote IS NOT NULL THEN		--VERIFICO QUE EL ID LOTE NO EXISTA EN LA TABLA DE LOTE_PNC, SI YA EXISTE ENTONCES NO HAGO NADA		SELECT 			COUNT(*) INTO contadorLoteEnLotePNC		FROM 			"CPSAA"."GESAC_MOV_LOTE_PNC"		WHERE			"SCMOL_IDE_LOTE_K" = idLote;		--SI EL contadorLoteEnLotePNC ES 0 --> HAGO LA LOGICA, SINO NO HAGO NADA PORQUE ESE LOTE YA EXISTE EN LA TABLA LOTE_PNC		IF (contadorLoteEnLotePNC = 0) THEN					--OBTENGO EL NOMBRE DEL TIPO DE ENSAYO			SELECT 				MAE."SCMTE_NOM_TIPO_ENSA" INTO nombreTipoEnsayo			FROM				"CPSAA"."GESAC_MOV_ENSA" MOV INNER JOIN				"CPSAA"."GESAC_MAE_TIPO_ENSA" MAE ON				MOV."SCMTE_IDE_TIPO_ENSA_K" = MAE."SCMTE_IDE_TIPO_ENSA_K"			WHERE				MOV."SCMOE_IDE_ENSA_K" = NEW."SCMOE_IDE_ENSA_K";									--OBTENGO EL ID DE LA PLANTA ASOCIADA A ESE LOTE			  SELECT LOTE."SCMAP_IDE_PLAN_K" INTO idPlanta			  FROM				"CPSAA"."GESAC_MOV_LOTE" LOTE 			  WHERE				LOTE."SCMOL_IDE_LOTE_K" = idLote;			--VERIFICO SI EL TIPO DE ENSAYO DE ESE REGISTRO ES DE RESISTENCIA COMPRESION A 28 DIAS			IF nombreTipoEnsayo IS NOT NULL THEN				--IF(nombreTipoEnsayo = 'Resistencia Compresión a 28días' OR nombreTipoEnsayo = 'Resistencia Compresión 28días') THEN 							select nombreTipoEnsayoEsResistenciaCompresion28Dias(nombreTipoEnsayo) INTO esResistencia28Dias;				IF (esResistencia28Dias = 1) THEN					--VERIFICO SI EL ESTADO DEL REGISTRO QUE SE HA INSERTADO ES IGUAL A 'NOC'					IF (NEW."SCMOE_COC_EST_TRA" = 'NOC') THEN											--ACTUALIZO EL ESTADO DEL LOTE						--UPDATE "CPSAA"."GESAC_MOV_LOTE"						--SET							--"SCMOL_COC_EST" = 'NCO'						--WHERE							--"SCMOL_IDE_LOTE_K" = idLote;												--INSERTO UN REGISTRO EN LA TABLA LOTE_PNC						INSERT INTO "CPSAA"."GESAC_MOV_LOTE_PNC"(							"SCMOL_IDE_NUM_PNC_K", "SCMOL_FCH_HOR_REG", "SCMOL_IDE_LOTE_K", "SCMAP_IDE_PLAN_K")						VALUES ( (SELECT								coalesce(max(LOTE_PNC."SCMOL_IDE_NUM_PNC_K"),0) + 1							FROM								"CPSAA"."GESAC_MOV_LOTE_PNC" LOTE_PNC),(SELECT TIMESTAMP WITHOUT TIME ZONE 'now'), idLote, idPlanta);											ELSE						--VERIFICO SI HAY ENTRE TODOS LOS ENSAYOS POR LO MENOS UN ENSAYO CON ESTADO NO CONFORME (NOC)						  SELECT COUNT(*) INTO contadorEstadoNOC						FROM 							"CPSAA"."GESAC_MOV_ENSA"						WHERE							"SCMOM_IDE_MUES_K" = NEW."SCMOM_IDE_MUES_K"						AND							"SCMOE_COC_EST_TRA" = 'NOC';																IF (contadorEstadoNOC > 0) THEN						--TENGO QUE ACTUALIZAR Y TENGO QUE INSERTAR EN LA TABLA LOTE_PNC							IF idLote IS NOT NULL THEN							--ACTUALIZO EL ESTADO DEL LOTE								UPDATE "CPSAA"."GESAC_MOV_LOTE"								SET									"SCMOL_COC_EST" = 'NCO'								WHERE									"SCMOL_IDE_LOTE_K" = idLote;							--INSERTO UN REGISTRO EN LA TABLA LOTE_PNC								INSERT INTO "CPSAA"."GESAC_MOV_LOTE_PNC"(									"SCMOL_IDE_NUM_PNC_K", "SCMOL_FCH_HOR_REG", "SCMOL_IDE_LOTE_K", "SCMAP_IDE_PLAN_K")								VALUES ( (SELECT										coalesce(max(LOTE_PNC."SCMOL_IDE_NUM_PNC_K"),0) + 1									   FROM									   "CPSAA"."GESAC_MOV_LOTE_PNC" LOTE_PNC),(SELECT TIMESTAMP WITHOUT TIME ZONE 'now'), idLote, idPlanta);							END IF;										END IF;					END IF;				--SI EL ENSAYO QUE SE VA A INSERTAR NO ES DE RESISTENCIA COMPRESION A 28 DIAS, VALIDO QUE EXISTA EN LOS DEMAS ENSAYOS UNO DE RESISTENCIA COMPRESION A 28 DIAS				ELSE 					SELECT existeResistenciaCompresion28Dias(NEW."SCMOM_IDE_MUES_K") INTO esResistencia28Dias;					IF (esResistencia28Dias = 2) THEN--SI ES 2 --> EXISTE RESISTENCIA COMPRESION 28 DIAS PERO SU ESTADO NO ES 'NOC'						--VERIFICO SI HAY ENTRE TODOS LOS ENSAYOS POR LO MENOS UN ENSAYO CON ESTADO NO CONFORME (NOC)						  SELECT COUNT(*) INTO contadorEstadoNOC						FROM 							"CPSAA"."GESAC_MOV_ENSA"						WHERE							"SCMOM_IDE_MUES_K" = NEW."SCMOM_IDE_MUES_K"						AND							"SCMOE_COC_EST_TRA" = 'NOC';											IF (contadorEstadoNOC > 0) THEN							--TENGO QUE ACTUALIZAR Y TENGO QUE INSERTAR EN LA TABLA LOTE_PNC							--ACTUALIZO EL ESTADO DEL LOTE							UPDATE "CPSAA"."GESAC_MOV_LOTE"							SET								"SCMOL_COC_EST" = 'NCO'							WHERE								"SCMOL_IDE_LOTE_K" = idLote;							--INSERTO UN REGISTRO EN LA TABLA LOTE_PNC							INSERT INTO "CPSAA"."GESAC_MOV_LOTE_PNC"(								"SCMOL_IDE_NUM_PNC_K", "SCMOL_FCH_HOR_REG", "SCMOL_IDE_LOTE_K", "SCMAP_IDE_PLAN_K")							VALUES ( (SELECT									coalesce(max(LOTE_PNC."SCMOL_IDE_NUM_PNC_K"),0) + 1								   FROM								   "CPSAA"."GESAC_MOV_LOTE_PNC" LOTE_PNC),(SELECT TIMESTAMP WITHOUT TIME ZONE 'now'), idLote, idPlanta);						END IF;					END IF;				END IF;			END IF;		END IF;	END IF;	RETURN NULL;      END;$BODY$  LANGUAGE 'plpgsql' VOLATILE  COST 100;ALTER FUNCTION verificarlotepnc() OWNER TO postgres;
...
INSERT INTO "CPSAA"."GESAC_DET_PERM_ROL"(
            "SCDPR_IDE_PERM_ROL_K", "SCMAP_IDE_PERM_K", "SCMAR_IDE_ROL_K", 
            "SCDPR_FLG_ELI", "SCDPR_COC_EST")
    VALUES ((SELECT coalesce(max(PER."SCDPR_IDE_PERM_ROL_K"),0) + 1 FROM "CPSAA"."GESAC_DET_PERM_ROL" PER), (SELECT MAX("SCMAP_IDE_PERM_K") FROM "CPSAA"."GESAC_MAE_PERM"), (SELECT MAX("SCMAR_IDE_ROL_K") FROM "CPSAA"."GESAC_MAE_ROL"), 
            'N', 'ACT');
...
INSERT INTO "CPSAA"."GESAC_DET_ROL_PUE_TRA"(
    ...
    VALUES ((SELECT coalesce(max(TRA."SCDRP_IDE_ROL_PUE_TRA_K"),0) + 1 FROM "CPSAA"."GESAC_DET_ROL_PUE_TRA" TRA), (SELECT MAX("SCMAR_IDE_ROL_K") FROM "CPSAA"."GESAC_MAE_ROL"), 'A102', 
            'N', 'ACT');
