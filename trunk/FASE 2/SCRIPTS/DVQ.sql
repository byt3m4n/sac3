...
/**********************************************
FECHA		: 21-03-2012
DESCRIPCION	: CAMBIA DE ESTADO EL LOTE A NO CONFORME E INSERTA EN LA TABLA 
...
**********************************************/
	...
	--David Vivar Quinteros 22-03-2012 - CSTI
			...
			IF nombreTipoEnsayo IS NOT NULL THEN
				...
				IF (esResistencia28Dias = 1) THEN
					...
					IF (NEW."SCMOE_COC_EST_TRA" = 'NOC') THEN	
						...
						INSERT INTO "CPSAA"."GESAC_MOV_LOTE_PNC"("SCMOL_IDE_NUM_PNC_K", "SCMOL_FCH_HOR_REG", "SCMOL_IDE_LOTE_K", "SCMAP_IDE_PLAN_K")
						VALUES ( (SELECT coalesce(max(LOTE_PNC."SCMOL_IDE_NUM_PNC_K"),0) + 1 FROM "CPSAA"."GESAC_MOV_LOTE_PNC" LOTE_PNC),(SELECT TIMESTAMP WITHOUT TIME ZONE 'now'),idLote, idPlanta);
						ELSE
						--VERIFICO SI HAY ENTRE TODOS LOS ENSAYOS POR LO MENOS UN ENSAYO CON ESTADO NO CONFORME (NOC)
						  SELECT COUNT(*) INTO contadorEstadoNOC FROM "CPSAA"."GESAC_MOV_ENSA" WHERE "SCMOM_IDE_MUES_K" = NEW."SCMOM_IDE_MUES_K"
						AND "SCMOE_COC_EST_TRA" = 'NOC';
										
						IF (contadorEstadoNOC > 0) THEN
						--TENGO QUE ACTUALIZAR Y TENGO QUE INSERTAR EN LA TABLA LOTE_PNC
							IF idLote IS NOT NULL THEN
							--ACTUALIZO EL ESTADO DEL LOTE
								UPDATE "CPSAA"."GESAC_MOV_LOTE" SET "SCMOL_COC_EST" = 'NCO'
								WHERE "SCMOL_IDE_LOTE_K" = idLote;
						...
						END IF;
					END IF;
				--SI EL ENSAYO QUE SE VA A INSERTAR NO ES DE ESISTENCIA COMPRESION A 28 DIAS, VALIDO QUE EXISTA EN LOS DEMAS ENSAYOS UNO DE RESISTENCIA COMPRESION A 28 DIAS
				ELSE 
					SELECT existeResistenciaCompresion28Dias(NEW."SCMOM_IDE_MUES_K") INTO esResistencia28Dias;
					IF (esResistencia28Dias = 2) THEN--SI ES 2 --> XISTE RESISTENCIA COMPRESION 28 DIAS PERO SU ESTADO NO ES 'NOC'
						--VERIFICO SI HAY ENTRE TODOS LOS ENSAYOS POR LO MENOS UN ENSAYO CON ESTADO NO CONFORME (NOC)
						  SELECT COUNT(*) INTO contadorEstadoNOC
						FROM "CPSAA"."GESAC_MOV_ENSA"
						WHERE "SCMOM_IDE_MUES_K" = NEW."SCMOM_IDE_MUES_K"
						AND "SCMOE_COC_EST_TRA" = 'NOC';
